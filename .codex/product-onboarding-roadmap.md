...
- [x] Nowy użytkownik może uruchomić w repo jedną komendę: `npx codex-memory install`.
...
- [x] Użytkownik ma komendy: `codex-memory status`, `doctor`, `start`, `stop`, `restart`, `uninstall`, `upgrade`.
...
- [x] Ponieważ `codex-mem` jest zajęte w npm, publiczny release używa dostępnego pakietu `codex-memory`.

## Publiczny kontrakt v1

- [x] `npx codex-memory install` jest głównym happy path.
- [x] `codex-memory install --yes` działa bez interakcji dla CI/smoke.
- [x] `codex-memory install --dev-toolkit` zachowuje obecne repo-local workflow i wypisuje komendy developerskie.
- [x] `codex-memory doctor` pokazuje status Node/npm, Bun, uv, API, DB, MCP, plugin config, hooks i portów.
- [x] `codex-memory status/start/stop/restart` zarządza lokalnym workerem przez PID file i logi w user cache, nie jako systemowy service w v1.
- [x] `codex-memory uninstall` usuwa konfigurację pluginu/hooków z bieżącego repo, ale domyślnie nie usuwa danych pamięci bez `--delete-data`.

## Taski implementacyjne

### 1. Packaging i entrypoint npm

- [x] Przekształcić pakiet CLI w publiczny pakiet `codex-memory` z binarką `codex-memory`.
...
- [x] Dodać `npm pack` smoke, który instaluje paczkę z lokalnego tarballa i uruchamia `codex-memory --version`.
...
- [x] Zmienić hook commands z repo-relatywnych ścieżek typu `python plugins/.../hook_memory.py` na stabilny entrypoint `codex-memory hook <event>`.
...
- [x] Dodać `codex-memory dev doctor`, które sprawdza lokalny checkout i obecne monorepo scripts.
- [x] Upewnić się, że zmiany instalatora nie łamią repo-local plugin scaffold.

### 6. Dokumentacja i UX

- [x] Przepisać początek README na flow: `npx codex-memory install`, `codex-memory status`, `codex-memory remember ...`, `codex-memory query ...`.
...
- [x] Dodać smoke test na temp repo: `npx --package <local-tarball> codex-memory install --yes`.
...
- [x] Wymagane komendy końcowe: `bun run typecheck`, `bun run cli:test`, `uv run --project apps/api pytest apps/api/tests`, `powershell -ExecutionPolicy Bypass -File infra/scripts/smoke.ps1`, `codex-memory doctor`.

## Założenia

- [x] Zakładamy dostępność pakietu npm `codex-memory`; jeśli dostępność nie jest potwierdzona bezpośrednio przed publikacją, release nie może zostać oznaczony jako gotowy.
