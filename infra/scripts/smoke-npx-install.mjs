...
try {
  ...
  assert(
    ...
    "install did not create .codex/mem.config.json",
  );
  assert(
    existsSync(pluginManifestPath),
    "install did not create user-level plugin manifest",
  );
  assert(
    existsSync(marketplacePath),
    "install did not create marketplace config",
  );
  ...
  run("npx", [
    ...
    "Smoke memory",
    "--context",
    "npx install smoke wrote this memory.",
  ]);
  const queryOutput = runCapture("npx", [
    "--yes",
    "--package",
    tarball,
    "codex-memory",
  ...
  assert(
    queryOutput.includes("Smoke memory"),
    "query did not return smoke memory",
  );
  const activeHook = runCapture(
    "npx",
    ["--yes", "--package", tarball, "codex-memory", "hook", "user-prompt"],
    JSON.stringify({ prompt: "Smoke memory" }),
  );
  assert(
    activeHook.includes("hookSpecificOutput") || activeHook.includes("memory"),
    "active hook did not produce hook output",
  );
  ...
  const mcpOutput = runCapture(
    "bun",
    [mcpServer],
    `${frame(initialize)}${frame(toolsList)}`,
    { CODEX_MEM_API_URL: apiUrl },
  );
  const messages = readFrames(mcpOutput);
  const tools = messages.at(-1)?.result?.tools ?? [];
  assert(
    tools.some((tool) => tool.name === "query_memory"),
    "MCP tools list did not include query_memory",
  );

  run("npx", ["--yes", "--package", tarball, "codex-memory", "status"]);
  run("npx", ["--yes", "--package", tarball, "codex-memory", "stop"]);
  const degradedHook = runCapture(
    "npx",
    ["--yes", "--package", tarball, "codex-memory", "hook", "user-prompt"],
    JSON.stringify({ prompt: "Smoke memory after stop" }),
  );
  assert(
    degradedHook.includes("degraded") || degradedHook.includes("unavailable"),
    "offline hook did not report degraded mode",
  );
} finally {
