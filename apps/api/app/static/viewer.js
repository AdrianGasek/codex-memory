...
async function loadSearch(filters = {}) {
  ...
  results.querySelectorAll("[data-memory-id]").forEach((button) => {
    button.addEventListener("click", () => loadEntryDetails(button.dataset.memoryId));
  });
...
async function loadEntryDetails(memoryId) {
  ...
  const [history, audit] = await Promise.all([historyResponse.json(), auditResponse.json()]);
...
async function loadHealth() {
  ...
  const body = await response.json();
  health.innerHTML = body.components.map((component) => `
    <div class="event">
...
async function loadDebug(filters = {}) {
  ...
  const params = new URLSearchParams({ query: filters.query || "", limit: "5" });
  if (filters.type) params.set("type", filters.type);
  if (filters.project) params.set("project", filters.project);
  for (const tag of filters.tags || []) params.append("tags", tag);
  const response = await fetch(`/memory/debug/search?${params}`);
  const body = await response.json();
  debugPanel.innerHTML = body.results.map(({ entry, score, reason, components }) => `
    <div class="debug-row">
      <div>
