...
export function ensureToolchain(options: BootstrapOptions = {}): Toolchain {
  ...
  const bun =
    findCommand("bun", runner, env) ?? bootstrapBun(runtimeDir, runner, env);
  const uv =
    findCommand("uv", runner, env) ?? bootstrapUv(runtimeDir, runner, env);
...
function bootstrapBun(
  ...
  if (!npm) {
    throw new Error(
      "Bun is not installed and npm was not found, so Codex-Mem cannot bootstrap Bun.",
    );
  }

  runOrThrow(
    runner(npm, ["install", "--prefix", toolsDir, "bun"], {
      env,
      stdio: "pipe",
      encoding: "utf8",
    }),
    "Could not bootstrap Bun with npm.",
  );
  const bin =
...
function bootstrapUv(
  ...
  const bin =
    process.platform === "win32" ? join(uvDir, "uv.exe") : join(uvDir, "uv");
  if (!existsSync(bin)) {
