...
export function uninstallCommand(args: string[]): void {
  ...
  const marketplace = JSON.parse(original) as {
    plugins?: Array<{ name?: string }>;
  };
  marketplace.plugins = (marketplace.plugins ?? []).filter(
    (plugin) => !["codex-memory", "codex-mem"].includes(String(plugin.name)),
  );
  ...
  console.log(
    `Codex-Mem plugin removed. Backup: ${relative(process.cwd(), backupPath) || backupPath}`,
  );
}
