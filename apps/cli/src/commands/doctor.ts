...
export async function doctorCommand(): Promise<void> {
  ...
  console.log(
    `DB: ${existsSync(join(runtimeDir, "data", "db")) ? "ok" : "missing"}`,
  );
  console.log(
    `MCP: ${existsSync(join(runtimeDir, "runtime", "mcp-server", "dist", "server.js")) ? "ok" : "missing"}`,
  );
  console.log(
    `Plugin config: ${existsSync(join(runtimeDir, "runtime", "plugin", ".codex-plugin", "plugin.json")) ? "ok" : "missing"}`,
  );
  console.log(
    `Hooks: ${existsSync(join(runtimeDir, "runtime", "plugin", "hooks.json")) ? "ok" : "missing"}`,
  );
