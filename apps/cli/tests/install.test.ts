...
describe("installCommand", () => {
  test("creates repo-local Codex-Mem config with local storage defaults", async () => {
    ...
    try {
      ...
      const config = JSON.parse(
        readFileSync(join(repo, ".codex", "mem.config.json"), "utf8"),
      );
  ...
  test("installs runtime assets outside the repo", async () => {
    ...
    try {
      ...
      expect(
        existsSync(join(runtimeHome, "runtime", "api", "app", "main.py")),
      ).toBe(true);
      expect(
        existsSync(
          join(runtimeHome, "runtime", "mcp-server", "dist", "server.js"),
        ),
      ).toBe(true);
      expect(
        existsSync(
          join(runtimeHome, "runtime", "plugin", "scripts", "hook_memory.py"),
        ),
      ).toBe(true);
      const mcp = JSON.parse(
        readFileSync(
          join(runtimeHome, "runtime", "plugin", ".mcp.json"),
          "utf8",
        ),
      );
      expect(mcp.mcpServers["codex-memory"].env.CODEX_MEM_API_URL).toBe(
        "http://127.0.0.1:8000",
      );
      expect(mcp.mcpServers["codex-memory"].args[0]).toContain(
        join("runtime", "mcp-server", "dist", "server.js"),
      );
      ...
      expect(marketplace.plugins[0].source.path).toBe(
        join(runtimeHome, "runtime", "plugin"),
      );
    } finally {
  ...
  test("rolls back marketplace when plugin health check fails", async () => {
    ...
    try {
      ...
      ).rejects.toThrow("plugin install failed health check");
  ...
  test("migrates existing dev-toolkit data without overwriting runtime data", async () => {
    ...
    try {
      ...
      writeFileSync(
        join(repo, "data", "db", "codex-mem.sqlite3"),
        "repo-db",
        "utf8",
      );
      const runtimeSource = fakeRuntimeSource(repo);
      ...
      expect(
        readFileSync(
          join(runtimeHome, "data", "db", "codex-mem.sqlite3"),
          "utf8",
        ),
      ).toBe("repo-db");
      writeFileSync(
  ...
  test("resolves installer storage paths for relative and absolute inputs", () => {
    const repo = "C:\\repo";
    expect(resolveConfigPath(repo, "data/db.sqlite3")).toBe(
      "C:\\repo\\data\\db.sqlite3",
    );
    expect(resolveConfigPath(repo, "C:\\data\\db.sqlite3")).toBe(
      "C:\\data\\db.sqlite3",
    );
  });
});
...
function fakeRuntimeSource(repo: string): string {
  ...
  writeFileSync(
    join(source, "plugin", "scripts", "hook_memory.py"),
    "",
    "utf8",
  );
  return source;
